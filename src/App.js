...
const DressesApp = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/reservations")
      ...
      .then(json => {
        updateState(draft => {
          draft.reservations = json.reservations;
        });
      });
  }, []);
  ...
  const ReservationForm = ({ reservationAction }) => {
    ...
    let reservationDressID;
    let reservationRentDate;

    const handleSubmitReservation = (event) => {
      event.preventDefault();
      reservationAction(reservationCustomerID.value,
                        reservationDressID.value,
    ...
    return (
      ...
      <br></br>
      <input ref={(r) => (reservationDressID = r)} type="text" placeholder="Dress ID"/>
      <br></br>
  ...
  const createReservationAPI = (customer_id, dress_id, rent_date) => {
    return fetch("http://localhost:5000/createReservation", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  const createCustomerAPI = (first_name, last_name, phone_number, size, wedding_date) => {
    return fetch("http://localhost:5000/createCustomer", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  const returnDressAPI = (reservation_id, return_on) => {
    return fetch("http://localhost:5000/return", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
